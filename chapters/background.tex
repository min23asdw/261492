...
\par WebRTC (Web Real-Time Communication) เป็น open-source  ที่ให้บริการ  web browsers
 ...
 ทำให้การ Communication ด้วยเสียงและวิดีโอได้ผ่าน  Peer-to-peer   โดย  ตรงตามรูป \ref{fig:webrtc structure}
...
\begin{figure}[h] 
  \begin{center} 
  % \includegraphics{pic/webrtc.png}
  \includegraphics[scale=0.3]{pic/mlp_1.png}
  % \cite{NeuralNetwork}
  \end{center}
...
ซึ่งมี function มากมายไม่ว่าจะเป็น Hyperbolic tangent Sigmoid หรือ ReLU
...
\begin{equation} output node_j =  Y_j =  y(v_i)  \end{equation}
 โดยที่ ReLU Activation function \begin{equation} ~~ y(v_i) = \max(0,v_i)   
  \end{equation}
...
เนื่องจาก Training dataset นั้นมีขนาดที่ใหญ่เกินไป จึงมีการแบ่ง dataset ให้เล็กลง ซึ่งจะมีคำศัพท์ ดังนี้

\begin{enumerate}
  \item Epoch :  โดยจาก Training dataset 1 Epoch คือการที่ dataset ทั้งหมด Passed forward และ backpropagation
...
\begin{figure}[h]
  \begin{center}
  % \includegraphics{pic/webrtc.png}
  \includegraphics[scale=0.3]{pic/cnn_1.png}
  \end{center}
...
\begin{figure}[h]
  \begin{center}
  % \includegraphics{pic/webrtc.png}
  \includegraphics[scale=0.45]{pic/cnn_nn.png}\cite{CNN_mlp}
  \end{center}
...
\begin{figure}[h]
  \begin{center}
  % \includegraphics{pic/webrtc.png}
  \includegraphics[scale=0.45]{pic/The-architecture-transfer-learning.png}\cite{Transfer}
  \end{center}
  ...
  A domain ${\mathcal {D}}$ consists of: a feature space ${\mathcal {X}}$ and a marginal probability distribution
  ...
  ${\displaystyle f:{\mathcal {X}}\rightarrow {\mathcal {Y}}}$. The function ${\displaystyle f}$ 
  is used to predict the corresponding label    ${\displaystyle f(x)}$ 
  of a new instance  ${\displaystyle x}$  .This task, denoted by ${\displaystyle {\mathcal {T}}=\{{\mathcal {Y}},f(x)\}}$,
...
a target domain ${\mathcal{D}_{T}}$ and learning task ${\mathcal{T}_{T}}$
...
${\displaystyle f_{T}(\cdot )}$ in ${\mathcal{D}_{T}}$
