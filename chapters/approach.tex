...
\begin{figure}[h]
  ...
  \caption[โครงสร้างของระบบ CapSnap]{โครงสร้างของระบบ CapSnap}
...
ข้อมูลที่ใช้ในการ ฝึกสอนโมเดล(Train model) ใช้รูปภาพจากสินค้าสินค้าจากห้อง 422 ทำการจัดเก็บข้อมูลรูปภาพ โดยใช้กล้องมือถือ
ในการถ่ายภาพโดยสินค้า 1 ชนิด(class) ทำการถ่ายภาพ 8 รูป ในมุมที่แตกต่างกัน
โดยโมเดลในโครงงานนี้จะใช้สินค้าทั้งหมด 51 class และเพิ่มอีก 1 class สำหรับพื้นหลัง (Background class)
รวมเป็น 52 class
จัดเก็บใน Google Drive ดังรูป  \ref{fig:Google Drive}  \\
\begin{figure}[ht]
  \begin{center}
...
\end{figure}
โดยทำการแยก รูปภาพตามหมวดหมู่ และทำการดึงข้อมูลมา ฝึกสอนโมเดล ผ่าน Google Colab
โดยในแต่ละรูป จะเป็นข้อมูล Array ขนาด 224x224x3 ตามข้อมูลของรูปภาพ

\subsection{K-Fold Cross Validation}

เพื่อตรวจสอบว่า โมเดล ที่เราทำการฝึกสอนมานั้นมีความแม่นยำเพียงพอ ไม่ว่าจะเจอชุดข้อมูล dataset ที่ข้อมูลแตกต่างกัน ในแต่ละ Fold ก็ยังสามารถมี Accuracy ที่มีความใกล้เคียงกัน ในแต่ละ fold 
โดยที่ โมเดลนั้นไม่แม่นยำแค่บาง dataset ซึ่งเป็นถือว่ามีการ bias ต่อ dataset
...
โดยใน dataset แต่ละ fold ก็จะทำการ Augmentation เพื่อเพิ่มจำนวนข้อมูลที่ใช้ในการ ฝึกสอนโมเดล ของแต่ละ fold
\begin{figure}[h]
  \begin{center}
...
โดย 1 รูปภาพที่ผ่านการ Generate datasets จะกลายเป็น 18 รูปภาพซึ่งมีความแตกต่างกันเล็กน้อย ตามรูป \ref{fig:Dataset generator}

\begin{figure}[h]
  \begin{center}

    \includegraphics[scale=0.3]{pic/model/lay_genmore.png}
  \end{center}
...
โมเดลในโครงงานนี้จะใช้ Xception Pre-trained ซึ่งเป็น Convolutional Neural Network ซึ่งมีความลึก 71 layers.
เป็นโมเดลที่ผ่านการจากรูปภาพต่าง ๆ มากกว่าล้านรูปภาพจาก ImageNet database 
ซึ่งจะมีโครงสร้างดังรูป \ref{fig:Xception}
 
\begin{figure}[ht]
\begin{center}
  \includegraphics[scale=0.35]{pic/model/x_1.png}\cite{Xception}
  \caption[Xception]{Xception}
  \label{fig:Xception}
\end{center}
\end{figure}


โดยการนำ Xception Model มาใช้ในการแยกคุณลักษณะเด่นของรูปภาพ และ สร้างโมเดล มาต่อท้ายเพื่อ
เรียนรู้ลักษณะเด่นจากที่ Xception ทำการแยกออกมาได้
 
ซึ่ง โมเดล ที่จะใช้ก็จะเป็น Xception Model ที่รับ Input ขนาด 224 x 224 x 3 ตามขนาดรูปภาพ 224x224 pixel และตาม Layer RGB

\newpage
โดย Xception เป็นโมเดล ตั้งต้นที่่ผ่าน Pre-train จะทำหน้าที่เป็น Feature extraction  ลักษณะเด่นของรูปภาพ
โดย Xception จะถูก lock ค่าของ weight เอาไว้เพื่อไม่ไห้ค่าของ weight ที่ได้เรียนรู้ข้อมูลมาจาก ImageNet หายไป
ซึ่ง Xception  มี weight ทั้งหมด 20 ล้าน parameter

 
และนำ layer มาต่อท้ายโมเดล ด้วย Fully Connected Node ที่เรียกว่า Dense layer จำนวน 128 , 64 , 32 ,  64 node และ
layer สุดท้าย ซึ่งมีขนาดเท่ากับจำนวน ชนิดของสินค้า(Class) ซึ่งทำหน้าที่เป็น Classifier
...
โดยโครงสร้างของโมเดล จะเป็นดังรูป \ref{fig:Classification model}
...
\begin{table}[htbp]
  ...
  % \label{tab:แสดงการอธิบายกรณีการใช้งานกรณีแสกนสินค้าของระบบแอปพลิเคชันสำหรับโทรศัพท์มือถือ}
